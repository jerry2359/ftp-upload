  ...
  return new Promise(resolve => {
    ...
    const uploadSingle = (c, file) => {
      ...
      const handleUploadType = (file, uploadType) => {
        if (uploadType === 'skip') {
          // 某些服务器无法使用文件路径list，获取上层目录信息判断文件存在
          let remoteDestPath = remoteDestFile.split('/')
          let remoteDestName = remoteDestPath.pop()
          let remoteDestDir = remoteDestPath.join('/')
          c.list(remoteDestDir, (err, list) => {
            if (!err && list.length && list.filter(function(x){return x.name===remoteDestName}).length) {
                // 存在远程文件，跳过上传
                updateProgress()
            } else {
              // 不存在远程文件，执行上传文件
